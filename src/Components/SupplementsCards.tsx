...
const SupplementsCards = () => {
  ...
  interface Supplements {
    ...
    qte_stock: number;
    price: number;
    maker: Maker[]; // Assure-toi que cette interface correspond à la structure des données reçues.
  }
  ...
  useEffect(() => {
    axios.post("http://127.0.0.1:8741/api/login_check", {
  ...
  const fetchSupplementslist = (token) => {
    axios.get("http://127.0.0.1:8741/api/supplements", {
          ...
          <RouterLink key={supplement.id} to={`/supplements/${supplement.id}`} style={{ textDecoration: 'none' }}>
            <Card sx={{ width: 250, bgcolor: '#FFFFF', maxWidth: '100%', boxShadow: 'lg' }}>
              <AspectRatio minHeight="230px" sx={{ minWidth: 200 }}>
                <img src={supplement.img_supplements} loading="lazy" alt="" />
              </AspectRatio>
              <CardContent sx={{ textAlign: 'center' }}>
                ...
                <Typography level="title-lg" sx={{ mt: 1, fontWeight: 'xl', color: 'black' }}>
